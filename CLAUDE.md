...
- **Web routes and the landing page** — `/` is `pages/Home.tsx` (landing), `/map` is the Leaflet map, `/seasons` is the canvas viz; anything else redirects to `/`. Two consequences of `HashRouter` that bite on a content page: the router owns the URL hash, so in-page `<a href="#section">` anchors do **not** work (use `scrollIntoView`), and `og:` URLs in `web/index.html` must be absolute because crawlers do not run JS. Map and Seasons are `React.lazy` chunks and `leaflet/dist/leaflet.css` is imported from `pages/Map.tsx`, not `main.tsx` — `/` is the entry route and must not ship Leaflet or the Seasons canvas. For the same reason the Home hero uses 400 colors inlined in `src/heroDots.ts` rather than fetching the 3.4 MB `season_photos.json`. The dark palette lives in a `:root` block in `src/styles.css`, but `Seasons.tsx` repeats those same hex values as canvas `fillStyle` strings (a 2D context cannot read CSS variables), so changing a token means grepping `Seasons.tsx` too. Landing-page body copy must stay at or above `--muted-2` (`#7c828f`, 5.0:1 on the background); the retired `#6b7280` was 4.0:1 and failed WCAG AA.

- **Naming the archive's author** — the photographs and captions are **Alexey Sergeev's** (texasmushrooms.org, math faculty at Texas A&M). The repo itself only ever contains the URL slug `asergeev`, so his name came from the live site, not from anything committed here. Keep the credit and the "not affiliated with him" line on the landing page and in the footer. Do not conflate the three photo counts: **19,156 pictures / 1,232 species** is what his site reports, **21,528 photos / 1,420 day pages** is what the scrape captured, and **8,800 photos / 718 species / 46 cells** is the filtered 2018–2024 analysis set that the Map and Seasons pages actually show.
